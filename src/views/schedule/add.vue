...
import { addSchedule, getDict } from "@/api/schedule";
...
const closeTab = () => {
  // 跳转至最后一个标签页
  useMultiTagsStoreHook().handleTags("splice", "/schedule/add");
  router.push({
...
const addScheduleDetail = async () => {
  ...
  try {
    ...
    const data = await addSchedule(scheduleData.value);
    console.log("新建任务信息----", data);
...
<template>
  <div class="main">
    ...
    <div
      ...
      class="w-full 0m0b-"
    >
      <el-empty description="暂无业务系统数据" v-show="sysInfo.length === 0" />
      <template v-if="sysInfo.length > 0">
        <el-form
          :model="scheduleData"
        ...
        >
          ...
          <el-form-item style="margin-left: 100px; margin-top: 20px">
            <el-button type="primary" @click="addScheduleDetail()"
              >提交</el-button
            >
