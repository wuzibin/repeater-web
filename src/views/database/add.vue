...
import { addDBInfo, checkDBConn } from "@/api/database";
...
const closeTab = () => {
  // 跳转至最后一个标签页
  useMultiTagsStoreHook().handleTags("splice", "/database/add");
  router.push({
    path: multiTags.value[(multiTags as any).value.length - 1].path
  });
};

const addDBDetail = async () => {
  try {
    const data = await addDBInfo(dbData.value);
    ...
    >
      ...
      >
        ...
        <el-form-item style="margin-left: 100px; margin-top: 20px">
          <el-button type="primary" @click="addDBDetail()">新增</el-button>
